...
-- Creating the database in posture
CREATE DATABASE ecomm_cust_db;

—USE load_olist_data.py to load data into database

-- Connect to 
\c ecomm_cust_db

-- Checking what Indexes exists 
SELECT 
    schemaname,
    tablename,
    indexname,
...
-- Clean up some loading errors 
ALTER TABLE order_items DROP COLUMN "Unnamed: 0";
ALTER TABLE order_payments DROP COLUMN "Unnamed: 0";
ALTER TABLE products DROP COLUMN "Unnamed: 0";

-- From customers table
CREATE INDEX idx_customer_id_customer ON customers(customer_id);
...
CREATE INDEX idx_seller_id_order_items ON order_items(seller_id);

-- From order payments table
CREATE INDEX idx_order_id_order_payments ON order_payments(order_id);

-- From order review table
CREATE INDEX idx_order_id_order_reviews ON order_reviews(order_id);
...
-- From sellers table
CREATE INDEX idx_seller_id_sellers ON sellers(seller_id);

-- Implement Star Schema
CREATE SCHEMA IF NOT EXISTS ecommerce_analytics;
SET search_path TO ecommerce_analytics;  -- Uses \c ecomm as default schema 
