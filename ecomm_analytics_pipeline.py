...
# 1. EXTRACT & LOAD DIMENSION TABLES
...
def load_sales_fact():
    ...
    insert_query = """
        ...
        FROM order_items oi
        JOIN orders_dataset o ON oi.order_id = o.order_id
        JOIN analytics.dim_customer dc ON o.customer_id = dc.customer_id
        JOIN analytics.dim_product dp ON oi.product_id = dp.product_id
        JOIN analytics.dim_seller ds ON oi.seller_id = ds.seller_id
        LEFT JOIN (
            SELECT 
                customer_id,
                order_id,
...
def setup_ab_test():
    ...
    cur.execute("""
        INSERT INTO analytics.dim_ab_test 
    ...
    # Create variants
    cur.execute("""
        INSERT INTO analytics.dim_test_variant 
        ...
        FROM analytics.dim_ab_test t WHERE t.test_id = 'coupon_promo_v1'
...
def assign_customers_to_variants():
    ...
    cur.execute("""
        INSERT INTO analytics.fact_ab_test_events 
        ...
        CROSS JOIN analytics.dim_ab_test t
        JOIN analytics.dim_test_variant v ON t.test_key = v.test_key
        ...
        END
        AND NOT EXISTS (
            SELECT 1 FROM analytics.fact_ab_test_events e 
...
def bayesian_ab_analysis():
    ...
    query = """
        ...
        FROM analytics.fact_ab_test_events e
        JOIN analytics.dim_ab_test t ON e.test_key = t.test_key
        JOIN analytics.dim_test_variant v ON e.variant_key = v.variant_key
        WHERE t.test_id = 'coupon_promo_v1'
